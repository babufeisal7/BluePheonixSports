    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 transition-opacity duration-300 ease-in-out">
      <div className="relative bg-white p-4 sm:p-8 rounded-lg shadow-lg max-w-lg sm:max-w-2xl mx-auto transition-transform transform scale-95 md:scale-100">
        ...
        <div className="flex flex-col sm:flex-row items-center">
          ...
          <div className="w-full sm:w-1/2 p-2 sm:p-4">
            <h2 className="text-2xl sm:text-3xl font-bold text-primary">{player.name}</h2>
            <h3 className="text-lg sm:text-xl text-secondary-foreground">{player.position}</h3>
            <p className="mt-2 sm:mt-4 text-sm sm:text-base text-muted-foreground">{player.bio}</p>
